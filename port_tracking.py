...
if __name__ == '__main__':
    ...
    today_folder = os.path.join(data_folder, datetime.today().strftime('%Y-%m-%d'))
    ...
    save_csv(inport_vessels, os.path.join(today_folder, inport_filename))

    print("\nArrivals extraction")
    arrival_vessels = crawler.get_arrivals(port_id)
    arrival_filename = f"port_{port_id}_{today_tms}_arrivals.csv"
    save_csv(arrival_vessels, os.path.join(today_folder, arrival_filename))
    ...
    save_csv(inport_calls, os.path.join(today_folder, calls_filename))
    ...
    for vessel in arrival_vessels:
        ...
        save_csv(ports, os.path.join(today_folder, port_filename))

        events = crawler.get_vessel_events(vessel['mmsi'])
        event_filename = f"vessel_{vessel['mmsi']}_{today_tms}_events.csv"
        save_csv(events, os.path.join(today_folder, event_filename))